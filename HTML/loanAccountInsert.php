<?php
    ...
    while(($row = mysqli_fetch_array($result)) && $runsql === true)
    {
        ...
        if(($customer1 === $customer1post || $customer1 === $customer2post) && ($customer2 === $customer1post || $customer2 === $customer2post) && ($account_type == 'Loan'))
        {
            $runsql = false;
        }
        else
    ...
    if($runsql === true)
    {
        ...
        if(!isset($_POST['customer2']) || $_POST['customer2'] === "")
        {
            ...
            $sql = "INSERT into account (customer_id_1, customer_id_2, account_type) VALUES ('$_POST[customer1]', null, 'Loan')";
        }
        else
        {
        ...
        if(!isset($_POST['customer2'])  || $_POST['customer2'] === "")
        {
            $sql = "SELECT account_id FROM account WHERE customer_id_1 = '$_POST[customer1]' AND customer_id_2 IS null AND account_type = 'Loan'";
        }
        else
        {
